// Professional Excel Budget Planner Application
class ProfessionalBudgetPlanner {
    constructor() {
        this.transactions = [];
        this.budgetLimits = {};
        this.categories = {
    ...
    init() {
        console.log('Initializing Professional Budget Planner...');
    ...
    setupEventListeners() {
        ...
        // Tab switching - Make sure this works
        ...
        // Add transaction button - Make sure this works
    ...
    switchTab(tabName) {
        ...
        const tabButtons = document.querySelectorAll('.nav-tab');
        console.log('Removing active class from', tabButtons.length, 'tabs');
        ...
        console.log('Removing active class from', contentSections.length, 'content sections');
        ...
        setTimeout(() => {
            console.log('Rendering content for tab:', tabName);
            switch(tabName) {
                case 'dashboard':
                    this.updateDashboard();
                    break;
    ...
    updateDashboard() {
        ...
        if (incomeEl) {
            ...
            console.log('Updated income:', incomeEl.textContent);
        }
        if (expensesEl) {
            expensesEl.textContent = this.formatCurrency(totalExpenses);
            console.log('Updated expenses:', expensesEl.textContent);
        }
        if (balanceEl) {
    ...
    renderTransactions() {
        ...
        tbody.innerHTML = sortedTransactions.map((transaction, index) => `
            <tr>
                ...
                <td><span class="status status--${transaction.type === 'income' ? 'success' : 'warning'}">${transaction.type}</span></td>
                <td>${transaction.category}</td>
                <td>${transaction.description}</td>
                <td style="text-align: right; font-family: -apple-system, BlinkMacSystemFont, 'SF Mono', monospace; color: ${transaction.type === 'income' ? '#28a745' : '#dc3545'}; font-weight: 600;">
        ...
        container.innerHTML = this.categories.expenses.map(category => {
            ...
            return `
                <div class="budget-input-row">
                    ...
                    <input type="number" name="${category}" value="${currentLimit}" min="0" step="100" class="form-control budget-input-field">
                </div>
            `;
        }).join('');
    }
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        ...
        app = new ProfessionalBudgetPlanner();
